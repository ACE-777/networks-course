...
1. Использует ли ваш браузер HTTP версии 1.0 или 1.1? Какая версия HTTP работает на
   сервере?
   - Мой браузер использует версию http 1.1
   - На сервере также рабоатет версия 1.1
...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - Вижу только HTTP язык принимает браузер.
   - Браузер предоставляет с какого порта был отправлен запрос, IP адресс пользователя, сколько байтов было передано, во сколько по GMT был отпрвлен get запрос, сколько времени прошло с запроса.
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Видно явно,что вернул text/html, который у нас на странице в браузере:
   ![img_3.png](img_3.png)
...
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - Да, есть такая строка. Судя по всему там информация о том, когда был совершон последний такой же get запрос
![img_4.png](img_4.png)
...
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - Not Modified ,а код 304 соответственно.
   - Явно сервер не вернул содержимое файла.
...
1. Сколько сообщений HTTP GET отправил ваш браузер? Какой номер пакета в трассировке
   ...
   - браузер chrome отправил два сообщения get (также тестировал на edge, там только один get был).
   - 5472 и 6105
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - GET было 3.
   - На сам сайт с текстом и изображениями (http://gaia.cs.umass.edu/wireshark-labs/HTTP-wireshark-file4.html) и на сайты, где лежат изображения эти (http://gaia.cs.umass.edu/pearson.png ; http://kurose.cslash.net/8E_cover_small.jpg).
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Визуально они были загружены последовательно, на практике мы видим что было 2 запроса послано get для получения картинок. Также это может быть связанно с размерностью картинок, которая явно отличается. 
