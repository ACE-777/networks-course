...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - SYN: 2640584052
   - Это можно определить по наличию установленного бита SYN в заголовке TCP-сегмента, который является 
   частью TCP-сообщения
![img_8.png](img_8.png)
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - 1399116224
   - Значение подтверждения (Acknowledgment Number) в SYNACK-сегменте равно 1. Это значение указывает на порядковый 
   номер следующего ожидаемого байта от клиента
   - Сервер gaia.cs.umass.edu определил значение подтверждения на основе порядкового номера последнего полученного байта
   от клиента. В данном случае, так как сервер отвечает на SYN-сегмент, то подтверждение установлено в значение, которое
   на единицу больше порядкового номера, указанного в SYN-сегменте
   - Это SYNACK-сегмент определяется наличием установленных битов SYN и ACK в заголовке TCP-сегмента
   ![img_9.png](img_9.png)
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - у post 148090, у остальных 1
   - каждый сегмент был отправлен друг за другом с различным интервалом
   - через какое-то различное время 
   - RTT равно 0.113400000
...
Для проверки перейти в каждой из консоли в дирректорию lab07:
```bash
cd lab07
```

Затем последовательно по порядку в консолях исполнить:
```bash
